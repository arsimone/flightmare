...
def parser():
    ...
    parser.add_argument('--seed', type=int, default=0,
                        help="Random seed")
    parser.add_argument('-w', '--weight', type=str, default='./saved/snaga/2021-04-15-17-24-18/tcn_velocity_tracker_81000000_steps.zip',#velocity_tracker_1400000_steps.zip', #velocity_tracker_3000000_steps.zip',#velocity_tracker_50000000_steps.zip',
...
def main():
    ...
    object_density_fractions = np.linspace(0, 1.0, num=env.num_envs, dtype=np.float32)
    if args.render:
    ...
    if args.train:
        ...
        if args.file:
            model = PPO(
                ...
                vf_coef=0.5,
                max_grad_norm=0.5,
                batch_size=env.num_envs * 800,
                n_epochs=10,
                clip_range=0.2,
                verbose=2,
            )
            model = model.load(args.file, env)
